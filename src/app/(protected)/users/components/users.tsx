...
export default function UsersTable() {
  ...
  const fetchSupabaseUsers = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(
          "id, first_name, last_name, email, created_at, last_sign_in_at",
        );
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      alert(
        `Failed to add user: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    }
  };
        ...
        <TableBody>
          {users.map((user) => (
            <TableRow key={user.id}>
              ...
              <TableCell>
                {`${dataFallback(user.first_name)} ${dataFallback(user.last_name)}` ||
                  "N/A"}
              </TableCell>
              <TableCell>{user.email || "N/A"}</TableCell>
              <TableCell>
                {dataFallback(
                  formatUnixDate(new Date(user.created_at).getTime()),
                ) || "N/A"}
              </TableCell>
              <TableCell>
                {dataFallback(
                  user.last_sign_in_at
                    ? formatUnixDate(new Date(user.last_sign_in_at).getTime())
                    : "Never",
                )}
              </TableCell>
              <TableCell>
